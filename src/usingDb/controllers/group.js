...
export default class GroupControllers {
        ...
        try {
            ...
            if (rowCount === 0) {
                ...
                res
                    .status(201)
                    .json(
                        {'status': 201, data, 'message': `${groupData.name} Created Successfully`}
                    )
            } else {
                res
                    .status(401)
                    .json({'status': 401, 'Error': 'Group Already Exist'})
            }
    ...
    static async getAllGroupInfo(req, res) {
        ...
        if (rowCount === 0) {
            res
                .status(400)
                .json({'status': 400, 'message': 'No Record'});
        } else {
            const data = rows;
            res
                .status(200)
        ...
        if (evaluateCreator.rowCount !== 1) {
            ...
            if (rowCount > 0) {
                res
                    .status(401)
                    .json(
                        {'status': 401, 'message': 'Group Name Already Exist Choose another Name'}
                    )
            } else {
                ...
                return res
                    .status(201)
                    .json({'status': 201, 'message': `Group name updated to ${name}`});
            }
      }
   }
